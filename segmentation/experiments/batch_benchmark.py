...
# ===========================================================================
DEFAULT_SEARCH_DIR = Path(__file__).parent.parent / "results"  # segmentation/results/
...
def try_load_model(model_path: Path):
    ...
    if suffix == ".keras":
        attempts.append(
            lambda: tf.keras.models.load_model(str(model_path), compile=False, safe_mode=False)
        )
